...
TOKEN = os.environ.get("BOT_TOKEN", "8917175933:AAGkwJnpCSvLMH3J1NA__QRWBeJPmeJ8lrw")
...
def build_report(d):
    ...
    if d.get('late_bus') and d['late_bus'].lower() != 'لا':
        summary += (
    ...
    if d.get('extra') and d['extra'].lower() != 'لا':
        summary += f"\n{d['extra']}"

    if d.get('actions') and d['actions'].lower() != 'لا':
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✈️ *مولّد التقرير الميداني*\n\nأهلاً! سأسألك عن بيانات الرحلة خطوة بخطوة.\n\nأرسل /cancel في أي وقت للإلغاء.\n\n*رقم الرحلة؟*\nمثال: SV1084",
        parse_mode="Markdown"
    )
    return FLIGHT_NUM
